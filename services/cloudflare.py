...
def register(
    public_key,
    private_key,
    device_model=f"{fake.company()} {fake.country()}",
    referrer="",
    proxy=None,
) -> Account:
    """
    ...
    data = {
        ...
        "fcm_token": "{}:APA91b{}".format(
            install_id, fake.pystr(min_chars=134, max_chars=134)
        ),
    ...
    return gen_account_from_response(
        response.json(), private_key=private_key, referrer=referrer
    )
...
def get_account(account: Account, proxy=None) -> dict:
    ...
    response = SESSION.get(
        f"{API_URL}/{API_VERSION}/reg/{account.account_id}",
...
def get_client_config(proxy=None) -> dict:
    ...
    response = SESSION.get(f"{API_URL}/{API_VERSION}/client_config", proxies=proxy)
    response.raise_for_status()
